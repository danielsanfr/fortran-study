...
integer message,esco,nl,nc,j,i,ii,m,iostat,ierr,tam(256,10),r,t,aux(256),a,b,iret,pro
...
external adiciona_dado,adiciona_lista,apaga_dado,apaga_lista,carrega_lista,procura_dado,area_restrita
...
select case(esco)
	...
	case(1)
		...
		call area_restrita(pro)

		if(pro == 1) call adiciona_lista(nl,nc,arq,tit,lista,listaux)		
	...
	case(3)
		...
		if(pro == 1) call apaga_lista(lista)
	...
	case(4)
		...
		if(pro == 1) call adiciona_dado(lista,nc,nl,arq,tit)
	...
	case(6)
		...
		if(pro == 1) call apaga_dado(lista,arq,arqux,tit,nl,nc,aux)
